...
class Attention(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, gn_group=2, groups=1, reduction=0.0625, kernel_num=4, min_channel=16):
    ...
    def forward(self, x):
        ...
        x = self.bn(x)
        x = self.relu(x)
        return self.func_channel(x), self.func_filter(x), self.func_kernel(x)

class PMDC(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, gn_group=2, stride=1, padding=1, dilation=1, groups=1,
                 reduction=0.0625, kernel_num=4):
        super(PMDC, self).__init__()
        ...
        self.attention = Attention(in_planes, out_planes, kernel_size, gn_group=gn_group,
